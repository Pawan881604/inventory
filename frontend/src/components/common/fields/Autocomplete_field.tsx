        ...
        render={({ field }) => (
          <>
            <Autocomplete
              ...
              inputProps={{
                classNames: {
                  input: `ml-1 dark:text-dark_color ${hasError ? 'border-red-500' : ''}`, // Add red border if error
                  inputWrapper: "h-[48px]",
                },
              }}
              listboxProps={{
                ...
                <AutocompleteItem key={item.value} textValue={item.value}>
                  <div className="flex justify-between items-center">
                    <div className="flex gap-2 items-center">
                      <Avatar alt={item.value} className="flex-shrink-0" size="sm" src={item.value} />
                      <div className="flex flex-col">
                        <span className="text-small dark:text-dark_color">{item.label}</span>
